...
class AttQF(nn.Module):
    def __init__(self, in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True, mode='CRC', negative_slope=0.2):
        super(AttQF, self).__init__()
...
class RDAB(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv',
        ...
        self.m_up3 = nn.ModuleList([upsample_block(nc[3], nc[2], bias=True, mode='2'),
                                  *[AttQF(nc[2], nc[2], bias=True, mode='C' + act_mode + 'C') for _ in range(nb)]])

        self.m_up2 = nn.ModuleList([upsample_block(nc[2], nc[1], bias=True, mode='2'),
                                  *[AttQF(nc[1], nc[1], bias=True, mode='C' + act_mode + 'C') for _ in range(nb)]])

        self.m_up1 = nn.ModuleList([upsample_block(nc[1], nc[0], bias=True, mode='2'),
                                  *[AttQF(nc[0], nc[0], bias=True, mode='C' + act_mode + 'C') for _ in range(nb)]])


        self.m_tail = conv(nc[0], out_nc, bias=True, mode='C')


        self.qf_pred = SN(*[ResBlock(nc[3], nc[3], bias=True, mode='C' + act_mode + 'C') for _ in range(nb)],
        ...
        self.qf_embed = SN(torch.nn.Linear(1, 512),
                                  nn.ReLU(),
                                  torch.nn.Linear(512, 512),
                                  nn.ReLU(),
                                  torch.nn.Linear(512, 512),
        ...
        self.to_gamma_3 = SN(torch.nn.Linear(512, nc[2]),nn.Sigmoid())
        self.to_beta_3 =  SN(torch.nn.Linear(512, nc[2]),nn.Tanh())
        self.to_gamma_2 = SN(torch.nn.Linear(512, nc[1]),nn.Sigmoid())
        self.to_beta_2 =  SN(torch.nn.Linear(512, nc[1]),nn.Tanh())
        self.to_gamma_1 = SN(torch.nn.Linear(512, nc[0]),nn.Sigmoid())
        self.to_beta_1 =  SN(torch.nn.Linear(512, nc[0]),nn.Tanh())
